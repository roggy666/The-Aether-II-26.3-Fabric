...
public record EffectBuildupPredicate(Holder<MobEffect> effect, Optional<Integer> buildupLimit) implements EntitySubPredicate {
    public static final Codec<EffectBuildupPredicate> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
    ...
    public boolean matches(Entity entity, ServerLevel level, @Nullable Vec3 position) {
        EffectBuildupInstance instance = entity.getAttachedOrCreate(AetherIIDataAttachments.EFFECTS_SYSTEM).getActiveBuildups().get(this.effect());
        if (instance != null) {

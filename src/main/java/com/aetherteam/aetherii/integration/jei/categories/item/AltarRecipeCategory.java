...
public class AltarRecipeCategory extends AbstractRecipeCategory<AltarEnchantingRecipe> {
    ...
    @Override
    public void draw(AltarEnchantingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor graphics, double mouseX, double mouseY) {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, AltarEnchantingRecipe recipe, IFocusGroup focuses) {
        if (recipe.input().getCustomIngredient() == null && recipe.input().acceptsItem(recipe.result().typeHolder())) {
        ...
        Direction slotDirection = Direction.WEST;
        for (int index = 1; index <= recipe.fuelCount(); index++) {
            builder.addSlot(RecipeIngredientRole.INPUT, slotX, slotY).add(AetherIIItems.AMBROSIUM_SHARD).setStandardSlotBackground();
            if (index % 2 == 0) {
                slotDirection = slotDirection.getCounterClockWise();
            }
            slotX += (32 * slotDirection.getStepX());

...
public class MoaEyesLayer extends RenderLayer<MoaRenderState, EntityModel<MoaRenderState>> {
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight, MoaRenderState moa, float v, float v1) {
        if (!moa.isInvisible && !moa.hasSpecialTexture()) {
            ...
            submitNodeCollector.submitModel(this.getParentModel(), moa, poseStack, AetherIIRenderTypes.entityDitherNoCull(AetherIIAtlases.MOA_EYES_SHEET), packedLight, LivingEntityRenderer.getOverlayCoords(moa, 0.0F), ARGB.colorFromFloat(opacity, 1.0F, 1.0F, 1.0F), sprite, moa.outlineColor);
        }
    }
}

    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight, MoaRenderState moa, float v, float v1) {
        if (!moa.isInvisible) {
            ...
            if (moa.saddlebag.is(AetherIIItems.MOA_SADDLEBAG)) {
                ...
                submitNodeCollector.submitModel(this.saddlebag, moa, poseStack, AetherIIRenderTypes.entityDitherNoCull(SADDLEBAG_TEXTURE), packedLight, OverlayTexture.NO_OVERLAY, color, null, moa.outlineColor);
            } else if (moa.saddlebag.is(AetherIIItems.LARGE_MOA_SADDLEBAG)) {
                this.largeSaddlebag.setupAnim(moa);
                submitNodeCollector.submitModel(this.largeSaddlebag, moa, poseStack, AetherIIRenderTypes.entityDitherNoCull(LARGE_SADDLEBAG_TEXTURE), packedLight, OverlayTexture.NO_OVERLAY, color, null, moa.outlineColor);
            }
        }
    }
}

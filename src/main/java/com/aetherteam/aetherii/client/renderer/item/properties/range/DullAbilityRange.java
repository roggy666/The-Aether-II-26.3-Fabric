...
public class DullAbilityRange implements RangeSelectItemModelProperty {
    ...
    public float get(ItemStack itemStack, @Nullable ClientLevel clientLevel, @Nullable ItemOwner owner, int i) {
        return owner == null ? 0.0F : owner instanceof Player player && !player.getAttachedOrCreate(AetherIIDataAttachments.ABILITY_BEHAVIOR).getCanRefuelAbilities().get(itemStack.typeHolder()) ? 1.0F : 0.0F;
    }

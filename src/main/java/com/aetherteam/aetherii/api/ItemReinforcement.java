...
public record ItemReinforcement(Upgrade... upgrades) {
    ...
    public static abstract class ComponentOperation<V> {
        ...
        public static final Codec<ComponentOperation> CODEC = OperationType.CODEC.dispatch(ComponentOperation::type, type -> type.codec.fieldOf("value"));
        public static StreamCodec<RegistryFriendlyByteBuf, ComponentOperation> STREAM_CODEC = OperationType.STREAM_CODEC.dispatch(ComponentOperation::type, type -> type.streamCodec);
        ...
        protected static <T> MapCodec<TypedDataComponent<T>> makeCustomDataValueCodec(DataComponentType<T> type) {
        ...
        public enum OperationType implements StringRepresentable {
            ...
            final StreamCodec<RegistryFriendlyByteBuf, ? extends ComponentOperation> streamCodec;

            OperationType(Codec<? extends ComponentOperation> codec, StreamCodec<RegistryFriendlyByteBuf, ? extends ComponentOperation> streamCodec) {
                this.codec = codec;
    ...
    public static class Add<N extends Number> extends ComponentOperation<N> {
        ...
        public ItemStack modifyComponent(ItemStack stack) {
            N component = stack.get(this.componentInfo.type());
        ...
        public N add(N a, N b) {
            return (N) switch (a) {
                case Integer n -> n + b.intValue();
                case Long n -> n + b.longValue();
                case Short n -> n + b.shortValue();
                case Float n -> n + b.floatValue();
                case Double n -> n + b.doubleValue();
                default -> null;
            };
        }
    ...
    public static class Set<V extends Object> extends ComponentOperation<V> {
        ...
        @Override
        public ItemStack modifyComponent(ItemStack stack) {
            V component = stack.get(this.componentInfo.type());
            stack.set(this.componentInfo.type(), this.componentInfo.value());
            return stack;
        }

...
public class SnowDecorator extends TreeDecorator {
    ...
    public void place(TreeDecorator.Context context) {
        ...
        for (BlockPos leafPos : Util.shuffledCopy(context.leaves(), random)) {
            ...
            if (heightmapPos.getY() == relativePos.getY()) {
                context.setBlock(relativePos, BlockStateProvider.simple(AetherIIBlocks.ARCTIC_SNOW).getState(context.level(), random, relativePos));
    ...
    protected TreeDecoratorType<?> type() {
        return AetherIITreeDecoratorTypes.SNOW;
    }
}

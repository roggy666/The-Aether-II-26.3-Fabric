...
public class AlterGroundTagDecorator extends TreeDecorator {
    ...
    protected TreeDecoratorType<?> type() {
        return AetherIITreeDecoratorTypes.ALTER_GROUND_TAG;
    }
    ...
    public void place(TreeDecorator.Context context) {
        ...
        if (!list.isEmpty()) {
            ...
            list.stream().filter(p_69310_ -> p_69310_.getY() == i).forEach(p_225978_ -> {
                this.placeCircle(context, p_225978_.west().north());
                this.placeCircle(context, p_225978_.east(2).north());
                this.placeCircle(context, p_225978_.west().south(2));
                this.placeCircle(context, p_225978_.east(2).south(2));
                ...
                for (int j = 0; j < 5; j++) {
                    ...
                    if (l == 0 || l == 7 || i1 == 0 || i1 == 7) {
                        this.placeCircle(context, p_225978_.offset(-3 + l, 0, -3 + i1));
                    }
                }
            });
        }
    ...
    private void placeBlockAt(TreeDecorator.Context context, BlockPos pos) {
        for (int i = 2; i >= -3; i--) {
            ...
            if (context.level().isStateAtPosition(abovePos, (state) -> state.is(this.replaceTag))) {
                context.setBlock(abovePos, this.provider.getState(context.level(), context.random(), pos));
                break;
            }

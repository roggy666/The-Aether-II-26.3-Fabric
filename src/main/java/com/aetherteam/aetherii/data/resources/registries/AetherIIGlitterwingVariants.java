...
public class AetherIIGlitterwingVariants {
    ...
    private static PriorityProvider.Selector<SpawnContext, SpawnCondition> biomes(BootstrapContext<GlitterwingVariant> context, TagKey<Biome> biomeTag) {
        return new PriorityProvider.Selector<>(biomeCheck(context, biomeTag), 1);
    }

    private static BiomeCheck biomeCheck(BootstrapContext<GlitterwingVariant> context, TagKey<Biome> biomeTag) {
    ...
    private static void register(BootstrapContext<GlitterwingVariant> context, ResourceKey<GlitterwingVariant> key, float wingXOffset, float wingZRotation, boolean emissive, SpawnPrioritySelectors spawnSelectors) {
        register(context, key, spawnSelectors, wingXOffset, wingZRotation, emissive);
    }

    private static void register(BootstrapContext<GlitterwingVariant> context, ResourceKey<GlitterwingVariant> key, float wingXOffset, float wingZRotation, SpawnPrioritySelectors spawnSelectors) {
        register(context, key, spawnSelectors, wingXOffset, wingZRotation, false);
    }

    private static void register(BootstrapContext<GlitterwingVariant> context, ResourceKey<GlitterwingVariant> key, float wingXOffset, float wingZRotation, boolean emissive, PriorityProvider.Selector<SpawnContext, SpawnCondition>... spawnSelectors) {
        register(context, key, new SpawnPrioritySelectors(List.of(spawnSelectors)), wingXOffset, wingZRotation, emissive);
    }

    private static void register(BootstrapContext<GlitterwingVariant> context, ResourceKey<GlitterwingVariant> key, float wingXOffset, float wingZRotation, PriorityProvider.Selector<SpawnContext, SpawnCondition>... spawnSelectors) {
